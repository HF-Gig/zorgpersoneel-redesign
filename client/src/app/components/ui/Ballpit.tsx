...
class W {
  ...
  update(e) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      if (t.obstacles && Array.isArray(t.obstacles)) {
        for (const obs of t.obstacles) {
          const dx = I.x - obs.x;
          const dy = I.y - obs.y;
          const dz = I.z - (obs.z || 0);
          const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
          const sumRadius = radius + obs.radius;
          if (dist < sumRadius) {
            const overlap = sumRadius - dist;
            j.set(dx, dy, dz).normalize().multiplyScalar(overlap);
            I.add(j);
            const normalX = dx / (dist || 1);
            const normalY = dy / (dist || 1);
            const normalZ = dz / (dist || 1);
            const dotProduct = B.x * normalX + B.y * normalY + B.z * normalZ;
            if (dotProduct < 0) {
              B.x -= 2 * dotProduct * normalX * t.wallBounce;
              B.y -= 2 * dotProduct * normalY * t.wallBounce;
              B.z -= 2 * dotProduct * normalZ * t.wallBounce;
            }
          }
        }
      }
      ...
      const t = (function (e) {
        ...
        return {
          ...
          getColorAt: function (ratio, out = new l()) {
            const numColors = i.length;
            const index = Math.min(numColors - 1, Math.floor(ratio * numColors));
            const selectedColor = i[index];
            out.copy(selectedColor);
            return out;
          }
        };
      })(e);
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    if (spheresInstanceRef.current && spheresInstanceRef.current.spheres) {
      spheresInstanceRef.current.spheres.config.obstacles = props.obstacles;
    }
  }, [props.obstacles]);
