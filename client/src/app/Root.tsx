...
function GlobalStyles() {
  return (
    <style>{`
      ...
      .gradient-text {
        background: linear-gradient(130deg, #C4643A 0%, #D4A853 60%, #C4643A 100%);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-4 flex items-center justify-between">
        <Link to="/" className="font-display text-2xl font-bold text-foreground shrink-0">
          Zorg<span className="text-primary">personeel</span><span className="text-accent">.</span>
        </Link>
          ...
          {NAV_LINKS.map(({ label, to }) => (
            <Link
              ...
              className={`text-sm font-medium transition-colors duration-200 whitespace-nowrap ${
                pathname === to ? "text-primary" : "text-foreground/60 hover:text-foreground"
              }`}
            >
              {label}
            </Link>
        ...
        <div className="hidden lg:flex items-center gap-3 shrink-0">
          <a href="tel:0478229003" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="grid md:grid-cols-4 gap-12 mb-12">
          <div className="md:col-span-2">
            <Link to="/" className="font-display text-2xl font-bold text-foreground mb-4 block">
              Zorg<span className="text-primary">personeel</span><span className="text-accent">.</span>
            </Link>
            <p className="text-muted-foreground leading-relaxed mb-6 max-w-xs text-sm">
              De toonaangevende matching-partner voor zorgpersoneel in Nederland. Snel, betrouwbaar en DUO-geverifieerd.
            </p>
            <div className="space-y-2.5">
              {[
                { icon: Mail, text: "info@zorgpersoneel.nl", href: "mailto:info@zorgpersoneel.nl" },
                { icon: Phone, text: "0478-229 003", href: "tel:0478229003" },
                { icon: MapPin, text: "Keizersveld 47D, 5801 AM Venray", href: "#" },
              ].map(({ icon: Ic, text, href }) => (
                <a key={text} href={href} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                  <Ic className="w-4 h-4 text-primary flex-shrink-0" />{text}
                </a>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {["Personeel aanvragen", "Aanmelden als professional", "Klantenservice", "Privacybeleid"].map(item => (
        ...
        <div className="border-t border-border pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted-foreground">© 2026 Zorgpersoneel.nl — Alle rechten voorbehouden</p>
          <p className="font-mono-label text-xs text-muted-foreground">Keizersveld 47D · Venray · Nederland</p>
        </div>
      </div>
    </footer>
  );
