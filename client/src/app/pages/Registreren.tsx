  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            "Er is iets misgegaan bij het verzenden van uw aanmelding.",
        );
      }
      ...
      toast.error("Fout bij aanmelden", {
        description:
          err.message ||
          "Kon de aanmelding niet verzenden. Probeer het later opnieuw.",
      });
    }
  };
    ...
    return (
      ...
      >
        ...
        <p className="text-muted-foreground leading-relaxed mb-8 max-w-md mx-auto">
          Uw aanmelding en documenten zijn veilig ontvangen door ons team. We
          nemen binnen 1-2 werkdagen contact met u op om uw profiel te
          bespreken.
        </p>
        <div className="bg-secondary rounded-2xl p-6 text-left mb-8 max-w-sm mx-auto">
          ...
          {[
            "Beoordeling van uw documenten",
            "Persoonlijk kennismakingsgesprek",
            "Welkom bij ons netwerk en starten met diensten",
          ].map((s, i) => (
        ...
        <button
          ...
          className="flex items-center gap-2 bg-primary text-primary-foreground px-8 py-3.5 rounded-full font-semibold hover:bg-primary/90 transition-all mx-auto"
        >
          Nieuwe aanmelding
        </button>
      </motion.div>
    ...
    >
      ...
      <div className="mb-8 border-b border-border pb-6">
        ...
        <p className="text-muted-foreground text-sm">
          Uw aanmelding wordt direct doorgestuurd naar{" "}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            {...register("voornaam", { required: "Verplicht veld" })}
            className={`zp-input ${errors.voornaam ? "error" : ""}`}
        ...
        <div>
          ...
          <input
            {...register("achternaam", { required: "Verplicht veld" })}
            className={`zp-input ${errors.achternaam ? "error" : ""}`}
        ...
        <div>
          ...
          <input
            {...register("woonplaats", { required: "Verplicht veld" })}
            className={`zp-input ${errors.woonplaats ? "error" : ""}`}
            placeholder="Uw stad of gemeente"
          />
          {errors.woonplaats && (
            <p className="text-sm text-red-500 mt-1">
              {errors.woonplaats.message}
            </p>
          )}
        </div>
        ...
        <div>
          <label className="block text-sm font-semibold text-foreground mb-2">
            Regio (provincie) <span className="text-primary">*</span>
          </label>
          <select
            {...register("regio", { required: "Verplicht veld" })}
            className={`zp-input ${errors.regio ? "error" : ""}`}
          >
          ...
          {errors.regio && (
            <p className="text-sm text-red-500 mt-1">{errors.regio.message}</p>
          )}
        </div>

        <div>
          <label className="block text-sm font-semibold text-foreground mb-2">
            Beroep / Specialisatie <span className="text-primary">*</span>
          </label>
          <select
            {...register("beroep", { required: "Verplicht veld" })}
            className={`zp-input ${errors.beroep ? "error" : ""}`}
          >
          ...
          {errors.beroep && (
            <p className="text-sm text-red-500 mt-1">{errors.beroep.message}</p>
          )}
        </div>

        <div>
          <label className="block text-sm font-semibold text-foreground mb-2">
            Jaren ervaring <span className="text-primary">*</span>
          </label>
          <input
            {...register("ervaring", {
              required: "Verplicht veld",
              min: { value: 0, message: "Minimaal 0" },
            })}
            type="number"
            min="0"
            className={`zp-input ${errors.ervaring ? "error" : ""}`}
            placeholder="Bijv. 5"
          />
          {errors.ervaring && (
            <p className="text-sm text-red-500 mt-1">
              {errors.ervaring.message}
            </p>
          )}
        </div>
        ...
        <div className="md:col-span-2">
          <label className="block text-sm font-semibold text-foreground mb-2">
            Over uzelf <span className="text-primary">*</span>
          </label>
          <textarea
            {...register("over_uzelf", { required: "Verplicht veld" })}
            className={`zp-input ${errors.over_uzelf ? "error" : ""}`}
            rows={4}
            placeholder="Vertel kort iets over uw ervaring, motivatie, of voorkeur voor type zorginstelling..."
          />
          {errors.over_uzelf && (
            <p className="text-sm text-red-500 mt-1">
              {errors.over_uzelf.message}
            </p>
          )}
        </div>
        ...
        <div>
          <label className="block text-sm font-semibold text-foreground mb-2">
            CV uploaden <span className="text-primary">*</span>
          </label>
          <div
        ...
        <div>
          <label className="block text-sm font-semibold text-foreground mb-2">
            Diploma uploaden <span className="text-primary">*</span>
          </label>
          <div
            ...
            {diplomaFile ? (
              <div className="flex items-center justify-between gap-2">
                <div className="text-left overflow-hidden">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {(diplomaFile.size / 1024).toFixed(0)} KB
                  </p>
                </div>
                <button
                  type="button"
          ...
          {diplomaError && (
            <p className="text-xs text-red-500 mt-1">{diplomaError}</p>
          )}
        </div>
          ...
          >
            ...
            <div>
              ...
              <p className="font-medium text-foreground text-sm">
                Klik om een of meerdere certificaten te uploaden
              </p>
        ...
        <div className="md:col-span-2">
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              {...register("akkoord", { required: "U dient akkoord te gaan" })}
              type="checkbox"
              className="mt-1 w-4 h-4 accent-[#ab5c9d] cursor-pointer"
            />
            <span className="text-sm text-foreground/80">
              ...
              en geef toestemming voor het verwerken van mijn gegevens ten
              behoeve van arbeidsbemiddeling.{" "}
              <span className="text-primary">*</span>
            </span>
          </label>
          {errors.akkoord && (
            <p className="text-sm text-red-500 mt-1">
              {errors.akkoord.message}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="mt-8 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <p className="text-xs text-muted-foreground max-w-sm">
          Uw documenten en gegevens worden uitsluitend gebruikt voor
          arbeidsbemiddeling. Nooit gedeeld met derden.
        </p>
        <button
          type="submit"
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
          ...
          >
            ...
            <p className="text-xl text-muted-foreground max-w-2xl leading-relaxed">
              Meld u aan, upload uw CV, diploma en certificaten en begin direct
              met het kiezen van diensten die bij u passen.
            </p>
          </motion.div>
        </div>
      </section>
            ...
            {[
              {
                ...
                body: "Kies zelf wanneer en hoeveel u werkt. Geen verplichtingen.",
              },
              {
                icon: Shield,
                title: "DUO verificatie",
                body: "Uw diploma's worden officieel geverifieerd via DUO voor maximale geloofwaardigheid.",
              },
              {
                icon: Heart,
                title: "Persoonlijke begeleiding",
                body: "Ons team staat voor u klaar met advies en ondersteuning bij vragen.",
              },
